...
func GetComponentAccessTokenHandler(c *gin.Context) {
	token, expiresIn, err := wx.GetComponentAccessToken()
	if err != nil {
	...
	c.JSON(http.StatusOK, errno.OK.WithData(gin.H{"componentAccessToken": token, "expiresIn": expiresIn}))
}

// GetAuthorizerAccessTokenHandler 获取AuthorizerAccessToken
func GetAuthorizerAccessTokenHandler(c *gin.Context) {
	token, expiresIn, err := wx.GetAuthorizerAccessToken(c.Query("appid"))
	if err != nil {
		c.JSON(http.StatusOK, errno.ErrSystemError.WithData(err.Error()))
		return
	}
	c.JSON(http.StatusOK, errno.OK.WithData(gin.H{"authorizerAccessToken": token, "expiresIn": expiresIn}))
}
