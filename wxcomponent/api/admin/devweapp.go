...
func getVisitStatus(appid string) (int, error) {
	req := struct{}{}
	_, body, err := wx.PostWxJsonWithAuthToken(appid, "/wxa/getvisitstatus", "", req)
	if err != nil {
		log.Error(err)
		return 0, err
	}
...
func getVersionInfo(appid string, resp *getVersionInfoResp) error {
	req := struct{}{}
	_, body, err := wx.PostWxJsonWithAuthToken(appid, "/wxa/getversioninfo", "", req)
	if err != nil {
		log.Error(err)
		return err
	}
...
func speedUpAuditHandler(c *gin.Context) {
	...
	// 使用结构体而不是 gin.H 来避免序列化问题
	type speedUpAuditParams struct {
		AuditId int `json:"auditid" wx:"auditid"`
	}
	params := speedUpAuditParams{
		AuditId: auditId,
	}

	_, _, err = wx.PostWxJsonWithAuthToken(appid, "/wxa/speedupaudit", "", params)
	if err != nil {
		c.JSON(http.StatusOK, errno.ErrSystemError.WithData(err.Error()))
		return
	}
...
func releaseCodeHandler(c *gin.Context) {
	...
	req := struct{}{}
	//原本代码中使用了gin.H{}
	if _, _, err := wx.PostWxJsonWithAuthToken(appid, "/wxa/release", "", req); err != nil {
		log.Error(err.Error())
		c.JSON(http.StatusOK, errno.ErrSystemError.WithData(err.Error()))
		return
	}
...
func changeVisitStatusHandler(c *gin.Context) {
	...
	appid := c.DefaultQuery("appid", "")
	if appid == "" {
		c.JSON(http.StatusOK, errno.ErrInvalidParam.WithData("appid cannot be empty"))
		return
	}
	...
	// 3. 验证 action 参数
	if req.Action == "" {
		c.JSON(http.StatusOK, errno.ErrInvalidParam.WithData("action cannot be empty"))
		return
	}

	if req.Action != "open" && req.Action != "close" {
		c.JSON(http.StatusOK, errno.ErrInvalidParam.WithData("action must be 'open' or 'close'"))
		return
	}

	// 4. 构造请求参数 - 使用结构体而不是 map 来避免序列化问题
	type changeVisitStatusParams struct {
		Action string `json:"action" wx:"action"`
	}
	params := changeVisitStatusParams{
		Action: req.Action,
	}

	// 5. 调用微信接口
	_, _, err := wx.PostWxJsonWithAuthToken(appid, "/wxa/change_visitstatus", "", params)
	if err != nil {
