...
func getAccessTokenWithRetry(appid string, tokenType int) (string, uint64, error) {
	var token string
	var expiresIn uint64
	var err error
	for i := 0; i < 3; i++ {
		if token, expiresIn, err = getAccessToken(appid, tokenType); err != nil {
			log.Error(err)
...
func getAccessToken(appid string, tokenType int) (string, uint64, error) {
	// 读缓存
	cacheCli := db.GetCache()
	cacheKey := genTokenKey(appid, tokenType)
	if value, expiration, found := cacheCli.GetWithExpiration(cacheKey); found {
		log.Info("hit cache, token: ", value)
		return value.(string), uint64(time.Until(expiration).Seconds()), nil
	}
	...
	if found && record.Expiretime.After(time.Now()) {
		...
		return record.Token, uint64(cacheDuration / time.Second), nil
	}
	// 没有数据 重新获取
	...
	return token, uint64(cacheDuration / time.Second), err
}
