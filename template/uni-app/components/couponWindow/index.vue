		...
		<view class='coupon-window' :class='window==true?"on":""'>
			...
			<view class='couponWinList'>
				<view class='item acea-row row-between-wrapper' v-for="(item,index) in couponList" :key="index">
					<view class='money font-color' v-if="item.coupon_type == 2"><text class='num'>{{item.coupon_price / 10}}</text>{{$t(`折`)}}</view>
					<view class='money font-color' v-else>{{$t(`￥`)}}<text class='num'>{{item.coupon_price}}</text></view>
					<view class='text'>
						<view class='name' v-if="item.coupon_type == 2">{{$t(`购物满`)}}{{item.use_min_price}}{{$t(`打`)}}{{item.coupon_price / 10}}{{$t(`折`)}}</view>
						<view class='name' v-else>{{$t(`购物满`)}}{{item.use_min_price}}{{$t(`减`)}}{{item.coupon_price}}</view>
